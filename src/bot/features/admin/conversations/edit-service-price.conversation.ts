...
import { pendingServiceEdit } from '../menus/services-list.menu.js'
...
export async function editServicePriceConversation(conversation: AdminConversation, ctx: Context) {
    ...
    try {
        ...
        if (s) {
            await AppointmentService.updateService(s.id, s.name, s.duration_minutes, newPrice, s.is_active);
            await ctx.reply(t('edit-service-success', { price: newPrice }), {
                reply_markup: new InlineKeyboard().text(t('btn-back-to-admin'), "back_to_admin")
            });
        }
